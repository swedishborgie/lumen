...
| `--syslog-identifier`         | `LUMEN_SYSLOG_IDENTIFIER`         | _(binary name)_                | Syslog identifier for journald output; set to systemd unit instance name (e.g. `lumen@alice`)                           |
...
| `--dri-node`                  | `LUMEN_DRI_NODE`                  | _(auto-detect)_                | Path to DRI render node (e.g. `/dev/dri/renderD128`) for GPU acceleration                                               |
...
| `--auth-oauth2-subject`       | `LUMEN_AUTH_OAUTH2_SUBJECT`       | _(required for oauth2)_        | Expected `sub` claim in the validated ID token                                                                          |
| `--desktop`                   | `LUMEN_DESKTOP`                   | _(none)_                       | Named desktop environment preset: `labwc` or `kde`; sets default launch command and required env vars                   |
| `--launch`                    | `LUMEN_LAUNCH`                    | _(none)_                       | Shell command to launch as a Wayland client once the compositor is ready (passed to `/bin/sh -c`)                       |
