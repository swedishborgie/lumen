...
| [`tracing-journald`](https://crates.io/crates/tracing-journald)     | 0.3     | Tracing subscriber layer that emits structured log events directly to the systemd journal             | [docs.rs/tracing-journald](https://docs.rs/tracing-journald)     |
...
| [`wayland-client`](https://crates.io/crates/wayland-client)                 | 0.31    | Low-level Wayland client protocol bindings; used for clipboard bridging via the inner compositor connection                                                         | [docs.rs/wayland-client](https://docs.rs/wayland-client)                                        |
...
| [`xkbcommon`](https://crates.io/crates/xkbcommon)                           | 0.9     | Rust bindings to `libxkbcommon`; handles keyboard layout processing and keycode/keysym translation                                                                  | [docs.rs/xkbcommon](https://docs.rs/xkbcommon)                                                  |
...
| Crate                                                         | Version | Purpose                                                                                                                                             | Documentation                                              |
| ------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| [`ffmpeg-next`](https://crates.io/crates/ffmpeg-next)         | 8.x     | High-level Rust wrapper around the FFmpeg libraries; used for the VA-API hardware encoding pipeline (filter graph, codec context, frame allocation) | [docs.rs/ffmpeg-next](https://docs.rs/ffmpeg-next)         |
| [`ffmpeg-sys-next`](https://crates.io/crates/ffmpeg-sys-next) | 8.x     | Low-level FFI bindings to all FFmpeg C libraries (`libavcodec`, `libavfilter`, `libavutil`, etc.)                                                   | [docs.rs/ffmpeg-sys-next](https://docs.rs/ffmpeg-sys-next) |
...
| [`subtle`](https://crates.io/crates/subtle)               | 2.x     | Constant-time comparison primitives; used to compare bearer tokens without timing side-channels                                                                        | [docs.rs/subtle](https://docs.rs/subtle)               |
...
| Crate                                     | Version | Purpose                                                                                                                                          | Documentation                          |
| ----------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| [`evdev`](https://crates.io/crates/evdev) | 0.13    | Safe Rust bindings to the Linux `evdev` and `uinput` kernel interfaces; used to create virtual gamepad input devices and emit button/axis events | [docs.rs/evdev](https://docs.rs/evdev) |
| [`libc`](https://crates.io/crates/libc)   | 0.2     | C type definitions; used for low-level uinput/ioctl interoperability                                                                             | [docs.rs/libc](https://docs.rs/libc)   |
